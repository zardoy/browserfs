...
export default class FS {
     ...
     * @returns
     */
    initialize(rootFS: FileSystem, cred: Cred): FileSystem;
    /**
    ...
    write(fd: number, data: any, position: number | null, encoding: BufferEncoding, cb?: BFSThreeArgCallback<number, string>): void;
    /**
     * Write buffer to the file specified by `fd`.
     * Note that it is unsafe to use fs.write multiple times on the same file
    ...
    writeSync(fd: number, data: string, position?: number | null, encoding?: BufferEncoding): number;
    /**
     ...
     * @return [Number]
     */
    readSync(fd: number, length: number, position: number, encoding: BufferEncoding): string;
