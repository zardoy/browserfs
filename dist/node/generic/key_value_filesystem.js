...
function getEmptyDirNode() {
    ...
    return (emptyDirNode = buffer_1.Buffer.from('{}'));
}
/**
...
var SimpleSyncRWTransaction = /** @class */ (function () {
    ...
    SimpleSyncRWTransaction.prototype._has = function (key) {
...
var SyncKeyValueFileSystem = /** @class */ (function (_super) {
    ...
    SyncKeyValueFileSystem.prototype.accessSync = function (p, mode, cred) {
        var tx = this.store.beginTransaction('readonly'), node = this.findINode(tx, p);
        if (!node.toStats().hasAccess(mode, cred)) {
            throw api_error_1.ApiError.EACCES(p);
        }
    };
    SyncKeyValueFileSystem.prototype.renameSync = function (oldPath, newPath, cred) {
        ...
        if (!oldDirNode.toStats().hasAccess(stats_1.FilePerm.WRITE, cred)) {
            throw api_error_1.ApiError.EACCES(oldPath);
        }
        ...
        try {
            tx.put(oldDirNode.id, buffer_1.Buffer.from(JSON.stringify(oldDirList)), true);
            tx.put(newDirNode.id, buffer_1.Buffer.from(JSON.stringify(newDirList)), true);
        }
        catch (e) {
            tx.abort();
            throw e;
    ...
    SyncKeyValueFileSystem.prototype.statSync = function (p, isLstat, cred) {
        // Get the inode to the item, convert it into a Stats object.
        var stats = this.findINode(this.store.beginTransaction('readonly'), p).toStats();
        if (!stats.hasAccess(stats_1.FilePerm.READ, cred)) {
            throw api_error_1.ApiError.EACCES(p);
        }
        return stats;
    };
    SyncKeyValueFileSystem.prototype.createFileSync = function (p, flag, mode, cred) {
        var tx = this.store.beginTransaction('readwrite'), data = (0, util_1.emptyBuffer)(), newFile = this.commitNewFile(tx, p, stats_1.FileType.FILE, mode, cred, data);
        // Open the file.
        return new SyncKeyValueFile(this, p, flag, newFile.toStats(), data);
    };
    SyncKeyValueFileSystem.prototype.openFileSync = function (p, flag, cred) {
    ...
    SyncKeyValueFileSystem.prototype.unlinkSync = function (p, cred) {
        this.removeEntry(p, false, cred);
    };
    SyncKeyValueFileSystem.prototype.rmdirSync = function (p, cred) {
    ...
    SyncKeyValueFileSystem.prototype.mkdirSync = function (p, mode, cred) {
        var tx = this.store.beginTransaction('readwrite'), data = buffer_1.Buffer.from('{}');
        this.commitNewFile(tx, p, stats_1.FileType.DIRECTORY, mode, cred, data);
    };
    SyncKeyValueFileSystem.prototype.readdirSync = function (p, cred) {
        var tx = this.store.beginTransaction('readonly');
        var node = this.findINode(tx, p);
        if (!node.toStats().hasAccess(stats_1.FilePerm.READ, cred)) {
            throw api_error_1.ApiError.EACCES(p);
        }
        return Object.keys(this.getDirListing(tx, p, node));
    };
    SyncKeyValueFileSystem.prototype.chmodSync = function (p, isLchmod, mode, cred) {
        var path = isLchmod ? p : this.realpathSync(p, {}, cred);
        var fd = this.openFileSync(path, file_flag_1.FileFlag.getFileFlag('r+'), cred);
        fd.chmodSync(mode);
    };
    SyncKeyValueFileSystem.prototype.chownSync = function (p, isLchown, new_uid, new_gid, cred) {
        var path = isLchown ? p : this.realpathSync(p, {}, cred);
        var fd = this.openFileSync(path, file_flag_1.FileFlag.getFileFlag('r+'), cred);
    ...
    SyncKeyValueFileSystem.prototype.makeRootDirectory = function () {
        ...
        if (tx.get(ROOT_NODE_ID) === undefined) {
            ...
            dirInode = new inode_1.default(GenerateRandomID(), 4096, 511 | stats_1.FileType.DIRECTORY, currTime, currTime, currTime, 0, 0);
     ...
     */
    SyncKeyValueFileSystem.prototype.commitNewFile = function (tx, p, type, mode, cred, data) {
        ...
        if (!parentNode.toStats().hasAccess(4 /* Write */, cred)) {
            throw api_error_1.ApiError.EACCES(p);
        }
        ...
        try {
            ...
            fileNode = new inode_1.default(dataId, data.length, mode | type, currTime, currTime, currTime, cred.uid, cred.gid);
            // Commit file node.
            var fileNodeId = this.addNewNode(tx, fileNode.toBuffer());
            // Update and commit parent directory listing.
            dirListing[fname] = fileNodeId;
            tx.put(parentNode.id, buffer_1.Buffer.from(JSON.stringify(dirListing)), true);
        }
        catch (e) {
            tx.abort();
            throw e;
     ...
     * @todo Update mtime.
     */
    SyncKeyValueFileSystem.prototype.removeEntry = function (p, isDir, cred) {
        ...
        if (!fileNode.toStats().hasAccess(stats_1.FilePerm.WRITE, cred)) {
            throw api_error_1.ApiError.EACCES(p);
        }
        ...
        try {
            ...
            tx.put(parentNode.id, buffer_1.Buffer.from(JSON.stringify(parentListing)), true);
        }
        catch (e) {
            tx.abort();
            throw e;
...
var AsyncKeyValueFileSystem = /** @class */ (function (_super) {
    ...
    AsyncKeyValueFileSystem.prototype.access = function (p, mode, cred, cb) {
        var tx = this.store.beginTransaction('readonly');
        this.findINode(tx, p, function (inode) {
            if (inode instanceof inode_1.default) {
                if (!inode.toStats().hasAccess(mode, cred)) {
                    cb(api_error_1.ApiError.EACCES(p));
                }
            }
            else {
                cb(inode);
    ...
    AsyncKeyValueFileSystem.prototype.rename = function (oldPath, newPath, cred, cb) {
        var _this = this;
        ...
        var theOleSwitcharoo = function () {
            ...
            if (!oldParentList[oldName]) {
                ...
                var completeRename_1 = function () {
                    ...
                    tx.put(oldParentINode.id, buffer_1.Buffer.from(JSON.stringify(oldParentList)), true, function (e) {
                        if (noErrorTx(e, tx, cb)) {
                            if (oldParent === newParent) {
                                ...
                                tx.put(newParentINode.id, buffer_1.Buffer.from(JSON.stringify(newParentList)), true, function (e) {
                                    if (noErrorTx(e, tx, cb)) {
                                        tx.commit(cb);
                                    }
                                });
        ...
        var processInodeAndListings = function (p) {
            _this.findINodeAndDirListing(tx, p, function (e, node, dirList) {
                if (!node.toStats().hasAccess(stats_1.FilePerm.WRITE, cred)) {
                    throw api_error_1.ApiError.EACCES(p);
                }
                if (e) {
                    if (!errorOccurred) {
    ...
    AsyncKeyValueFileSystem.prototype.stat = function (p, isLstat, cred, cb) {
        var tx = this.store.beginTransaction('readonly');
        this.findINode(tx, p, function (e, inode) {
            if (noError(e, cb)) {
                var stats = inode.toStats();
                if (!stats.hasAccess(stats_1.FilePerm.READ, cred)) {
                    cb(api_error_1.ApiError.EACCES(p));
                }
                cb(null, stats);
    ...
    AsyncKeyValueFileSystem.prototype.createFile = function (p, flag, mode, cred, cb) {
        var _this = this;
        var tx = this.store.beginTransaction('readwrite'), data = (0, util_1.emptyBuffer)();
        this.commitNewFile(tx, p, stats_1.FileType.FILE, mode, cred, data, function (e, newFile) {
            if (noError(e, cb)) {
                cb(null, new AsyncKeyValueFile(_this, p, flag, newFile.toStats(), data));
            }
        });
    };
    AsyncKeyValueFileSystem.prototype.openFile = function (p, flag, cred, cb) {
        var _this = this;
        var tx = this.store.beginTransaction('readonly');
        // Step 1: Grab the file's inode.
        this.findINode(tx, p, function (e, inode) {
            if (noError(e, cb)) {
                var stats_2 = inode.toStats();
                if (!stats_2.hasAccess(flag.getMode(), cred)) {
                    cb(api_error_1.ApiError.EACCES(p));
                }
                ...
                tx.get(inode.id, function (e, data) {
                    if (noError(e, cb)) {
                        if (data === undefined) {
                            ...
                            cb(null, new AsyncKeyValueFile(_this, p, flag, stats_2, data));
                        }
                    }
                });
            }
    ...
    AsyncKeyValueFileSystem.prototype.unlink = function (p, cred, cb) {
        this.removeEntry(p, false, cred, cb);
    };
    AsyncKeyValueFileSystem.prototype.rmdir = function (p, cred, cb) {
        var _this = this;
        // Check first if directory is empty.
        this.readdir(p, cred, function (err, files) {
            if (err) {
                cb(err);
            }
                ...
                _this.removeEntry(p, true, cred, cb);
            }
        });
    };
    AsyncKeyValueFileSystem.prototype.mkdir = function (p, mode, cred, cb) {
        var tx = this.store.beginTransaction('readwrite'), data = buffer_1.Buffer.from('{}');
        this.commitNewFile(tx, p, stats_1.FileType.DIRECTORY, mode, cred, data, cb);
    };
    AsyncKeyValueFileSystem.prototype.readdir = function (p, cred, cb) {
        var _this = this;
        var tx = this.store.beginTransaction('readonly');
        this.findINode(tx, p, function (e, inode) {
            if (noError(e, cb)) {
                if (!inode.toStats().hasAccess(stats_1.FilePerm.READ, cred)) {
                    cb(api_error_1.ApiError.EACCES(p));
                }
    ...
    AsyncKeyValueFileSystem.prototype.makeRootDirectory = function (cb) {
        ...
        tx.get(ROOT_NODE_ID, function (e, data) {
            if (e || data === undefined) {
                ...
                var currTime = new Date().getTime(), 
                // Mode 0666, owned by root:root
                dirInode_1 = new inode_1.default(GenerateRandomID(), 4096, 511 | stats_1.FileType.DIRECTORY, currTime, currTime, currTime, 0, 0);
                // If the root doesn't exist, the first random ID shouldn't exist,
                // either.
     ...
     */
    AsyncKeyValueFileSystem.prototype.commitNewFile = function (tx, p, type, mode, cred, data, cb) {
        var _this = this;
        ...
        this.findINodeAndDirListing(tx, parentDir, function (e, parentNode, dirListing) {
            if (!parentNode.toStats().hasAccess(stats_1.FilePerm.WRITE, cred)) {
                cb(api_error_1.ApiError.EACCES(p));
            }
            if (noErrorTx(e, tx, cb)) {
                if (dirListing[fname]) {
                    ...
                    _this.addNewNode(tx, data, function (e, dataId) {
                        if (noErrorTx(e, tx, cb)) {
                            ...
                            var fileInode_1 = new inode_1.default(dataId, data.length, mode | type, currTime, currTime, currTime, cred.euid, cred.egid);
                            _this.addNewNode(tx, fileInode_1.toBuffer(), function (e, fileInodeId) {
                                if (noErrorTx(e, tx, cb)) {
                                    // Step 4: Update parent directory's listing.
                                    dirListing[fname] = fileInodeId;
                                    tx.put(parentNode.id, buffer_1.Buffer.from(JSON.stringify(dirListing)), true, function (e) {
                                        if (noErrorTx(e, tx, cb)) {
     ...
     * @todo Update mtime.
     */
    AsyncKeyValueFileSystem.prototype.removeEntry = function (p, isDir, cred, cb) {
        var _this = this;
                                    ...
                                    if (noErrorTx(e, tx, cb)) {
                                        ...
                                        tx.del(fileNodeId_1, function (e) {
                                            if (noErrorTx(e, tx, cb)) {
                                                ...
                                                tx.put(parentNode.id, buffer_1.Buffer.from(JSON.stringify(parentListing)), true, function (e) {
                                                    if (noErrorTx(e, tx, cb)) {
                                                        tx.commit(cb);
                                                    }
                                                });
