...
var HTTPRequest = /** @class */ (function (_super) {
    ...
    HTTPRequest.CreateAsync = function (opts) {
        var _this = this;
    ...
    HTTPRequest.prototype.stat = function (path, isLstat, cred, cb) {
        var inode = this._index.getInode(path);
        if (inode === null) {
        ...
        if (!inode.toStats().hasAccess(stats_1.FilePerm.READ, cred)) {
            return cb(api_error_1.ApiError.EACCES(path));
        }
        if ((0, file_index_1.isFileInode)(inode)) {
            stats = inode.getData();
            ...
            if (stats.size < 0) {
                this._requestFileSizeAsync(path, function (e, size) {
                    ...
                    stats.size = size;
                    cb(null, stats_1.default.clone(stats));
                });
            }
            else {
                cb(null, stats_1.default.clone(stats));
    ...
    HTTPRequest.prototype.statSync = function (path, isLstat, cred) {
        var inode = this._index.getInode(path);
        if (inode === null) {
            throw api_error_1.ApiError.ENOENT(path);
        }
        if (!inode.toStats().hasAccess(stats_1.FilePerm.READ, cred)) {
            throw api_error_1.ApiError.EACCES(path);
        }
        var stats;
        if ((0, file_index_1.isFileInode)(inode)) {
            stats = inode.getData();
    ...
    HTTPRequest.prototype.open = function (path, flags, mode, cred, cb) {
        ...
        if ((0, file_index_1.isFileInode)(inode) || (0, file_index_1.isDirInode)(inode)) {
            switch (flags.pathExistsAction()) {
                case file_flag_1.ActionType.THROW_EXCEPTION:
                case file_flag_1.ActionType.TRUNCATE_FILE:
                    return cb(api_error_1.ApiError.EEXIST(path));
                case file_flag_1.ActionType.NOP:
                    if ((0, file_index_1.isDirInode)(inode)) {
                        var stats_2 = inode.getStats();
                        return cb(null, new preload_file_1.NoSyncFile(self, path, flags, stats_2, stats_2.fileData || undefined));
                    }
                    var stats_3 = inode.getData();
                    // Use existing file contents.
                    // XXX: Uh, this maintains the previously-used flag.
                    if (stats_3.fileData) {
                        return cb(null, new preload_file_1.NoSyncFile(self, path, flags, stats_1.default.clone(stats_3), stats_3.fileData));
                    }
                    ...
                    this._requestFileAsync(path, 'buffer', function (err, buffer) {
                        ...
                        stats_3.size = buffer.length;
                        stats_3.fileData = buffer;
                        return cb(null, new preload_file_1.NoSyncFile(self, path, flags, stats_1.default.clone(stats_3), buffer));
                    });
                    break;
                default:
            ...
            return cb(api_error_1.ApiError.EPERM(path));
        }
    };
    HTTPRequest.prototype.openSync = function (path, flags, mode, cred) {
        // INVARIANT: You can't write to files on this file system.
        if (flags.isWriteable()) {
        ...
        if ((0, file_index_1.isFileInode)(inode) || (0, file_index_1.isDirInode)(inode)) {
            switch (flags.pathExistsAction()) {
                ...
                case file_flag_1.ActionType.NOP:
                    if ((0, file_index_1.isDirInode)(inode)) {
                        var stats_4 = inode.getStats();
                        return new preload_file_1.NoSyncFile(this, path, flags, stats_4, stats_4.fileData || undefined);
                    }
                    var stats = inode.getData();
                    // Use existing file contents.
                    // XXX: Uh, this maintains the previously-used flag.
                    if (stats.fileData) {
                        return new preload_file_1.NoSyncFile(this, path, flags, stats_1.default.clone(stats), stats.fileData);
                    }
                    // @todo be lazier about actually requesting the file
                    ...
                    stats.size = buffer.length;
                    stats.fileData = buffer;
                    return new preload_file_1.NoSyncFile(this, path, flags, stats_1.default.clone(stats), buffer);
                default:
                    throw new api_error_1.ApiError(api_error_1.ErrorCode.EINVAL, 'Invalid FileMode object.');
            }
        }
        else {
            throw api_error_1.ApiError.EPERM(path);
        }
    };
    HTTPRequest.prototype.readdir = function (path, cred, cb) {
        try {
    ...
    HTTPRequest.prototype.readdirSync = function (path, cred) {
        // Check if it exists.
        var inode = this._index.getInode(path);
        if (inode === null) {
            throw api_error_1.ApiError.ENOENT(path);
        }
        else if (!inode.toStats().hasAccess(stats_1.FilePerm.READ, cred)) {
            throw api_error_1.ApiError.EACCES(path);
        }
        else if ((0, file_index_1.isDirInode)(inode)) {
     ...
     */
    HTTPRequest.prototype.readFile = function (fname, encoding, flag, cred, cb) {
     ...
     */
    HTTPRequest.prototype.readFileSync = function (fname, encoding, flag, cred) {
        // Get file.
