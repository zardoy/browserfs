...
var MountableFileSystem = /** @class */ (function (_super) {
            ...
            if (imfs) {
                ...
                try {
                    for (var _i = 0, _a = Object.keys(opts); _i < _a.length; _i++) {
                        ...
                        fs_1.mount(mountPoint, opts[mountPoint], cred_1.default.Root);
                    }
                }
                catch (e) {
                    return cb(e);
    ...
    MountableFileSystem.CreateAsync = function (opts) {
        var _this = this;
     ...
     */
    MountableFileSystem.prototype.mount = function (mountPoint, fs, cred) {
        if (mountPoint[0] !== '/') {
            mountPoint = "/".concat(mountPoint);
        }
        mountPoint = path.resolve(mountPoint);
        if (this.mntMap[mountPoint]) {
            throw new api_error_1.ApiError(api_error_1.ErrorCode.EINVAL, 'Mount point ' + mountPoint + ' is already in use.');
        }
        (0, util_1.mkdirpSync)(mountPoint, 0x1ff, cred, this.rootFs);
        this.mntMap[mountPoint] = fs;
        this.mountList.push(mountPoint);
        this.mountList = this.mountList.sort(function (a, b) { return b.length - a.length; });
    };
    MountableFileSystem.prototype.umount = function (mountPoint, cred) {
        if (mountPoint[0] !== '/') {
            mountPoint = "/".concat(mountPoint);
        }
        mountPoint = path.resolve(mountPoint);
    ...
    // processing.
    MountableFileSystem.prototype.rename = function (oldPath, newPath, cred, cb) {
        var _this = this;
        // Scenario 1: old and new are on same FS.
        var fs1rv = this._getFs(oldPath);
        var fs2rv = this._getFs(newPath);
        if (fs1rv.fs === fs2rv.fs) {
            return fs1rv.fs.rename(fs1rv.path, fs2rv.path, cred, function (e) {
                if (e) {
    ...
    MountableFileSystem.prototype.renameSync = function (oldPath, newPath, cred) {
        // Scenario 1: old and new are on same FS.
        var fs1rv = this._getFs(oldPath);
        var fs2rv = this._getFs(newPath);
        if (fs1rv.fs === fs2rv.fs) {
    ...
    MountableFileSystem.prototype.readdirSync = function (p, cred) {
        ...
        try {
            var rv2_1 = fsInfo.fs.readdirSync(fsInfo.path, cred);
            if (rv === null) {
                return rv2_1;
            }
            else {
    ...
    MountableFileSystem.prototype.readdir = function (p, cred, cb) {
        var _this = this;
        var fsInfo = this._getFs(p);
        fsInfo.fs.readdir(fsInfo.path, cred, function (err, files) {
            if (fsInfo.fs !== _this.rootFs) {
                try {
                    var rv = _this.rootFs.readdirSync(p, cred);
                    if (files) {
                        // Filter out duplicates.
    ...
    MountableFileSystem.prototype.realpathSync = function (p, cache, cred) {
        var fsInfo = this._getFs(p);
        try {
            var mountedPath = fsInfo.fs.realpathSync(fsInfo.path, {}, cred);
    ...
    MountableFileSystem.prototype.realpath = function (p, cache, cred, cb) {
        var _this = this;
        var fsInfo = this._getFs(p);
        fsInfo.fs.realpath(fsInfo.path, {}, cred, function (err, rv) {
            if (err) {
    ...
    MountableFileSystem.prototype.rmdirSync = function (p, cred) {
        var fsInfo = this._getFs(p);
    ...
    MountableFileSystem.prototype.rmdir = function (p, cred, cb) {
        var _this = this;
        var fsInfo = this._getFs(p);
        if (this._containsMountPt(p)) {
            ...
            fsInfo.fs.rmdir(fsInfo.path, cred, function (err) {
