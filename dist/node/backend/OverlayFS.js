...
var UnlockedOverlayFS = /** @class */ (function (_super) {
    ...
    UnlockedOverlayFS.prototype._syncAsync = function (file, cb) {
        ...
        this.createParentDirectoriesAsync(file.getPath(), stats.getCred(0, 0), function (err) {
            if (err) {
                return cb(err);
            }
            _this._writable.writeFile(file.getPath(), file.getBuffer(), null, getFlag('w'), stats.mode, stats.getCred(0, 0), cb);
        });
    };
    ...
    UnlockedOverlayFS.prototype._initialize = function (cb) {
        ...
        this._writable.readFile(deletionLogPath, 'utf8', getFlag('r'), cred_1.default.Root, function (err, data) {
            if (err) {
    ...
    UnlockedOverlayFS.prototype.isReadOnly = function () {
        return false;
    };
    UnlockedOverlayFS.prototype.supportsSynch = function () {
        return this._readable.supportsSynch() && this._writable.supportsSynch();
    };
    UnlockedOverlayFS.prototype.supportsLinks = function () {
        return false;
    };
    UnlockedOverlayFS.prototype.supportsProps = function () {
        return this._readable.supportsProps() && this._writable.supportsProps();
    };
    UnlockedOverlayFS.prototype.getDeletionLog = function () {
        return this._deleteLog;
    };
    UnlockedOverlayFS.prototype.restoreDeletionLog = function (log, cred) {
        this._deleteLog = log;
        this._reparseDeletionLog();
        this.updateLog('', cred);
    };
    UnlockedOverlayFS.prototype.rename = function (oldPath, newPath, cred, cb) {
        var _this = this;
        ...
        this.stat(oldPath, false, cred, function (oldErr, oldStats) {
            if (oldErr) {
                return cb(oldErr);
            }
            return _this.stat(newPath, false, cred, function (newErr, newStats) {
                var self = _this;
                ...
                function copyDirContents(files) {
                    ...
                    self.rename(oldFile, newFile, cred, function (err) {
                        if (err) {
                            return cb(err);
                        }
                ...
                if (oldStats.isDirectory()) {
                    if (newErr) {
                        ...
                        return _this._writable.exists(oldPath, cred, function (exists) {
                            // simple case - both old and new are on the writable layer
                            if (exists) {
                                return _this._writable.rename(oldPath, newPath, cred, cb);
                            }
                            _this._writable.mkdir(newPath, mode, cred, function (mkdirErr) {
                                if (mkdirErr) {
                                    return cb(mkdirErr);
                                }
                                _this._readable.readdir(oldPath, cred, function (err, files) {
                                    if (err) {
                                        return cb();
                                    }
                                    copyDirContents(files);
                    ...
                    _this.readdir(newPath, cred, function (readdirErr, files) {
                ...
                _this.readFile(oldPath, null, getFlag('r'), cred, function (err, data) {
                    if (err) {
                        return cb(err);
                    }
                    return _this.writeFile(newPath, data, null, getFlag('w'), oldStats.mode, cred, function (err) {
                        if (err) {
                            return cb(err);
                        }
                        return _this.unlink(oldPath, cred, cb);
                    });
                });
            });
        });
    };
    UnlockedOverlayFS.prototype.renameSync = function (oldPath, newPath, cred) {
        var _this = this;
        ...
        var oldStats = this.statSync(oldPath, false, cred);
        if (oldStats.isDirectory()) {
            ...
            if (this.existsSync(newPath, cred)) {
                var stats = this.statSync(newPath, false, cred);
                mode = stats.mode;
            ...
            if (this._readable.existsSync(oldPath, cred)) {
                this._readable.readdirSync(oldPath, cred).forEach(function (name) {
                    // Recursion! Should work for any nested files / folders.
                    _this.renameSync(path.resolve(oldPath, name), path.resolve(newPath, name), cred);
                });
            }
        }
        else {
    ...
    UnlockedOverlayFS.prototype.stat = function (p, isLstat, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb)) {
            return;
        }
        this._writable.stat(p, isLstat, cred, function (err, stat) {
            if (err && err.errno === api_error_1.ErrorCode.ENOENT) {
                if (_this._deletedFiles[p]) {
                    cb(api_error_1.ApiError.ENOENT(p));
                }
                _this._readable.stat(p, isLstat, cred, function (err, stat) {
                    if (stat) {
                        // Make the oldStat's mode writable. Preserve the topmost
                        // part of the mode, which specifies if it is a file or a
                        // directory.
                        stat = stats_1.default.clone(stat);
    ...
    UnlockedOverlayFS.prototype.statSync = function (p, isLstat, cred) {
        this.checkInitialized();
        try {
            ...
            var oldStat = stats_1.default.clone(this._readable.statSync(p, isLstat, cred));
    ...
    UnlockedOverlayFS.prototype.open = function (p, flag, mode, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb) || this.checkPathAsync(p, cb)) {
            return;
        }
        this.stat(p, false, cred, function (err, stats) {
            if (stats) {
                switch (flag.pathExistsAction()) {
                    case file_flag_1.ActionType.TRUNCATE_FILE:
                        return _this.createParentDirectoriesAsync(p, cred, function (err) {
                            if (err) {
                                return cb(err);
                            }
                            _this._writable.open(p, flag, mode, cred, cb);
                        });
                    case file_flag_1.ActionType.NOP:
                        return _this._writable.exists(p, cred, function (exists) {
                            if (exists) {
                                _this._writable.open(p, flag, mode, cred, cb);
                            }
                            else {
                                ...
                                // the readable FS.
                                stats = stats_1.default.clone(stats);
                                stats.mode = mode;
                                _this._readable.readFile(p, null, getFlag('r'), cred, function (readFileErr, data) {
                                    if (readFileErr) {
                ...
                switch (flag.pathNotExistsAction()) {
                    case file_flag_1.ActionType.CREATE_FILE:
                        return _this.createParentDirectoriesAsync(p, cred, function (err) {
                            ...
                            return _this._writable.open(p, flag, mode, cred, cb);
                        });
                    default:
    ...
    UnlockedOverlayFS.prototype.openSync = function (p, flag, mode, cred) {
        this.checkInitialized();
        this.checkPath(p);
            ...
            switch (flag.pathExistsAction()) {
                ...
                case file_flag_1.ActionType.NOP:
                    if (this._writable.existsSync(p, cred)) {
                        ...
                        var buf = this._readable.readFileSync(p, null, getFlag('r'), cred);
                        var stats = stats_1.default.clone(this._readable.statSync(p, false, cred));
                        stats.mode = mode;
    ...
    UnlockedOverlayFS.prototype.unlink = function (p, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb) || this.checkPathAsync(p, cb)) {
            return;
        }
        this.exists(p, cred, function (exists) {
            if (!exists) {
                return cb(api_error_1.ApiError.ENOENT(p));
            }
            _this._writable.exists(p, cred, function (writableExists) {
                if (writableExists) {
                    return _this._writable.unlink(p, cred, function (err) {
                        if (err) {
                            return cb(err);
                        }
                        _this.exists(p, cred, function (readableExists) {
                            if (readableExists) {
                                _this.deletePath(p, cred);
                            }
                            cb(null);
                        });
                    });
    ...
    UnlockedOverlayFS.prototype.unlinkSync = function (p, cred) {
        this.checkInitialized();
        this.checkPath(p);
        if (this.existsSync(p, cred)) {
            if (this._writable.existsSync(p, cred)) {
    ...
    UnlockedOverlayFS.prototype.rmdir = function (p, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb)) {
            return;
        }
        var rmdirLower = function () {
            _this.readdir(p, cred, function (err, files) {
                if (err) {
                    return cb(err);
                }
                if (files.length) {
        ...
        this.exists(p, cred, function (exists) {
            ...
            _this._writable.exists(p, cred, function (writableExists) {
                if (writableExists) {
                    _this._writable.rmdir(p, cred, function (err) {
                        if (err) {
                            return cb(err);
                        }
                        _this._readable.exists(p, cred, function (readableExists) {
                            if (readableExists) {
                                rmdirLower();
                            }
                            else {
    ...
    UnlockedOverlayFS.prototype.rmdirSync = function (p, cred) {
        this.checkInitialized();
        if (this.existsSync(p, cred)) {
            if (this._writable.existsSync(p, cred)) {
    ...
    UnlockedOverlayFS.prototype.mkdir = function (p, mode, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb)) {
            return;
        }
        this.exists(p, cred, function (exists) {
            ...
            _this.createParentDirectoriesAsync(p, cred, function (err) {
                if (err) {
                    return cb(err);
                }
                _this._writable.mkdir(p, mode, cred, cb);
            });
        });
    };
    UnlockedOverlayFS.prototype.mkdirSync = function (p, mode, cred) {
        this.checkInitialized();
        if (this.existsSync(p, cred)) {
            throw api_error_1.ApiError.EEXIST(p);
        }
    ...
    UnlockedOverlayFS.prototype.readdir = function (p, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb)) {
            return;
        }
        this.stat(p, false, cred, function (err, dirStats) {
            if (err) {
                return cb(err);
            }
            ...
            _this._writable.readdir(p, cred, function (err, wFiles) {
                ...
                _this._readable.readdir(p, cred, function (err, rFiles) {
                    ...
                    var seenMap = {};
                    var filtered = wFiles.concat(rFiles.filter(function (fPath) { return !_this._deletedFiles["".concat(p, "/").concat(fPath)]; })).filter(function (fPath) {
                        // Remove duplicates.
    ...
    UnlockedOverlayFS.prototype.readdirSync = function (p, cred) {
        var _this = this;
        this.checkInitialized();
        var dirStats = this.statSync(p, false, cred);
        if (!dirStats.isDirectory()) {
        ...
        try {
            contents = contents.concat(this._readable.readdirSync(p, cred).filter(function (fPath) { return !_this._deletedFiles["".concat(p, "/").concat(fPath)]; }));
        }
        catch (e) {
            // NOP.
        }
    ...
    UnlockedOverlayFS.prototype.exists = function (p, cred, cb) {
        var _this = this;
        // Cannot pass an error back to callback, so throw an exception instead
        // if not initialized.
        this.checkInitialized();
        this._writable.exists(p, cred, function (existsWritable) {
            if (existsWritable) {
                return cb(true);
            }
            _this._readable.exists(p, cred, function (existsReadable) {
                cb(existsReadable && _this._deletedFiles[p] !== true);
            });
        });
    };
    UnlockedOverlayFS.prototype.existsSync = function (p, cred) {
        this.checkInitialized();
        return this._writable.existsSync(p, cred) || (this._readable.existsSync(p, cred) && this._deletedFiles[p] !== true);
    };
    UnlockedOverlayFS.prototype.chmod = function (p, isLchmod, mode, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb)) {
            return;
        }
        this.operateOnWritableAsync(p, cred, function (err) {
            if (err) {
                return cb(err);
            }
            else {
                _this._writable.chmod(p, isLchmod, mode, cred, cb);
            }
        });
    };
    UnlockedOverlayFS.prototype.chmodSync = function (p, isLchmod, mode, cred) {
        var _this = this;
        this.checkInitialized();
        this.operateOnWritable(p, cred, function () {
            _this._writable.chmodSync(p, isLchmod, mode, cred);
        });
    };
    UnlockedOverlayFS.prototype.chown = function (p, isLchmod, new_uid, new_gid, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb)) {
            return;
        }
        this.operateOnWritableAsync(p, cred, function (err) {
            if (err) {
                ...
                _this._writable.chown(p, isLchmod, new_uid, new_gid, cred, cb);
            }
        });
    };
    UnlockedOverlayFS.prototype.chownSync = function (p, isLchown, new_uid, new_gid, cred) {
        var _this = this;
        this.checkInitialized();
        this.operateOnWritable(p, cred, function () {
            _this._writable.chownSync(p, isLchown, new_uid, new_gid, cred);
        });
    };
    UnlockedOverlayFS.prototype.utimes = function (p, atime, mtime, cred, cb) {
        var _this = this;
        if (!this.checkInitAsync(cb)) {
            return;
        }
        this.operateOnWritableAsync(p, cred, function (err) {
            if (err) {
                ...
                _this._writable.utimes(p, atime, mtime, cred, cb);
            }
        });
    };
    UnlockedOverlayFS.prototype.utimesSync = function (p, atime, mtime, cred) {
        var _this = this;
        this.checkInitialized();
        this.operateOnWritable(p, cred, function () {
            _this._writable.utimesSync(p, atime, mtime, cred);
        });
    };
    UnlockedOverlayFS.prototype.deletePath = function (p, cred) {
        this._deletedFiles[p] = true;
        this.updateLog("d".concat(p, "\n"), cred);
    };
    UnlockedOverlayFS.prototype.updateLog = function (addition, cred) {
        var _this = this;
        ...
        if (this._deleteLogUpdatePending) {
            ...
            this._writable.writeFile(deletionLogPath, this._deleteLog, 'utf8', file_flag_1.FileFlag.getFileFlag('w'), 420, cred, function (e) {
                _this._deleteLogUpdatePending = false;
                if (e) {
    ...
    UnlockedOverlayFS.prototype.createParentDirectoriesAsync = function (p, cred, cb) {
        ...
        function createParents() {
            ...
            self._readable.stat(dir, false, cred, function (err, stats) {
                // stop if we couldn't read the dir
                if (!stats) {
                    return cb();
                }
                self._writable.mkdir(dir, stats.mode, cred, function (err) {
                    if (err) {
                        return cb(err);
                    }
                    createParents();
     ...
     */
    UnlockedOverlayFS.prototype.createParentDirectories = function (p, cred) {
        var _this = this;
        ...
        toCreate.forEach(function (p) {
            _this._writable.mkdirSync(p, _this.statSync(p, false, cred).mode, cred);
        });
    };
    /**
     * Helper function:
     ...
     */
    UnlockedOverlayFS.prototype.operateOnWritable = function (p, cred, f) {
        if (this.existsSync(p, cred)) {
            if (!this._writable.existsSync(p, cred)) {
    ...
    UnlockedOverlayFS.prototype.operateOnWritableAsync = function (p, cred, cb) {
        var _this = this;
        this.exists(p, cred, function (exists) {
            if (!exists) {
                return cb(api_error_1.ApiError.ENOENT(p));
            }
            _this._writable.exists(p, cred, function (existsWritable) {
                if (existsWritable) {
                    cb();
                }
                else {
                    return _this.copyToWritableAsync(p, cred, cb);
                }
            });
        });
    };
     ...
     */
    UnlockedOverlayFS.prototype.copyToWritable = function (p, cred) {
        var pStats = this.statSync(p, false, cred);
    ...
    UnlockedOverlayFS.prototype.copyToWritableAsync = function (p, cred, cb) {
        var _this = this;
        this.stat(p, false, cred, function (err, pStats) {
            if (err) {
                return cb(err);
            }
            if (pStats.isDirectory()) {
                return _this._writable.mkdir(p, pStats.mode, cred, cb);
            }
            // need to copy file.
            _this._readable.readFile(p, null, getFlag('r'), cred, function (err, data) {
                if (err) {
                    return cb(err);
                }
                _this.writeFile(p, data, null, getFlag('w'), pStats.mode, cred, cb);
            });
        });
    };
...
var OverlayFS = /** @class */ (function (_super) {
    ...
    OverlayFS.CreateAsync = function (opts) {
        var _this = this;
