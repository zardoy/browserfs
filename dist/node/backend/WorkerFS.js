...
var FileDescriptorArgumentConverter = /** @class */ (function () {
    ...
    FileDescriptorArgumentConverter.prototype.toRemoteArg = function (fd, p, flag, cb) {
        ...
        fd.stat(function (err, stats) {
            if (err) {
                ...
                if (flag.isReadable()) {
                    fd.read(buffer_1.Buffer.alloc(stats.size), 0, stats.size, 0, function (err, bytesRead, buff) {
                        if (err) {
                            cb(err);
                        }
                        else {
    ...
    FileDescriptorArgumentConverter.prototype._applyFdChanges = function (remoteFd, cb) {
        var fd = this._fileDescriptors[remoteFd.id], data = transferrableObjectToBuffer(remoteFd.data), remoteStats = stats_1.default.fromBuffer(transferrableObjectToBuffer(remoteFd.stat));
...
function statsRemote2Local(stats) {
    return stats_1.default.fromBuffer(transferrableObjectToBuffer(stats.statsData));
}
/**
 * @hidden
 */
...
var WorkerFS = /** @class */ (function (_super) {
    ...
    WorkerFS.CreateAsync = function (opts) {
        var _this = this;
    ...
    WorkerFS.attachRemoteListener = function (worker) {
        ...
        function argLocal2Remote(arg, requestArgs, cb) {
            switch (typeof arg) {
                case 'object':
                    if (arg instanceof stats_1.default) {
    ...
    WorkerFS.prototype.rename = function (oldPath, newPath, cred, cb) {
        this._rpc('rename', arguments);
    };
    WorkerFS.prototype.stat = function (p, isLstat, cred, cb) {
        this._rpc('stat', arguments);
    };
    WorkerFS.prototype.open = function (p, flag, mode, cred, cb) {
        this._rpc('open', arguments);
    };
    WorkerFS.prototype.unlink = function (p, cred, cb) {
        this._rpc('unlink', arguments);
    };
    WorkerFS.prototype.rmdir = function (p, cred, cb) {
        this._rpc('rmdir', arguments);
    };
    WorkerFS.prototype.mkdir = function (p, mode, cred, cb) {
        this._rpc('mkdir', arguments);
    };
    WorkerFS.prototype.readdir = function (p, cred, cb) {
        this._rpc('readdir', arguments);
    };
    WorkerFS.prototype.exists = function (p, cred, cb) {
        this._rpc('exists', arguments);
    };
    WorkerFS.prototype.realpath = function (p, cache, cred, cb) {
        this._rpc('realpath', arguments);
    };
    WorkerFS.prototype.truncate = function (p, len, cred, cb) {
        this._rpc('truncate', arguments);
    };
    WorkerFS.prototype.readFile = function (fname, encoding, flag, cred, cb) {
        this._rpc('readFile', arguments);
    };
    WorkerFS.prototype.writeFile = function (fname, data, encoding, flag, mode, cred, cb) {
        this._rpc('writeFile', arguments);
    };
    WorkerFS.prototype.appendFile = function (fname, data, encoding, flag, mode, cred, cb) {
        this._rpc('appendFile', arguments);
    };
    WorkerFS.prototype.chmod = function (p, isLchmod, mode, cred, cb) {
        this._rpc('chmod', arguments);
    };
    WorkerFS.prototype.chown = function (p, isLchown, new_uid, new_gid, cred, cb) {
        this._rpc('chown', arguments);
    };
    WorkerFS.prototype.utimes = function (p, atime, mtime, cred, cb) {
        this._rpc('utimes', arguments);
    };
    WorkerFS.prototype.link = function (srcpath, dstpath, cred, cb) {
        this._rpc('link', arguments);
    };
    WorkerFS.prototype.symlink = function (srcpath, dstpath, type, cred, cb) {
        this._rpc('symlink', arguments);
    };
    WorkerFS.prototype.readlink = function (p, cred, cb) {
    ...
    WorkerFS.prototype._initialize = function (cb) {
        ...
        if (!this._isInitialized) {
            var message = {
                ...
                method: 'probe',
                args: [
                    this._argLocal2Remote((0, util_1.emptyBuffer)()),
                ...
                if (typeof arg['type'] === 'number') {
                    ...
                    switch (specialArg.type) {
                        ...
                        case SpecialArgType.FD:
                            ...
                            return new WorkerFile(this, fdArg.path, file_flag_1.FileFlag.getFileFlag(fdArg.flag), stats_1.default.fromBuffer(transferrableObjectToBuffer(fdArg.stat)), fdArg.id, transferrableObjectToBuffer(fdArg.data));
                        case SpecialArgType.STATS:
                            return statsRemote2Local(specialArg);
                        case SpecialArgType.FILEFLAG:
                            return fileFlagRemote2Local(specialArg);
